...
exports.createEscalation = async (payload, userId) => {
    const result = await prisma.$queryRaw`
        ...
        VALUES (${payload.claim_id}, ${payload.escalation_level},
                ${payload.escalation_to_role_id || null}, ${payload.escalation_due_date},
                ${userId}, ${payload.remarks || null})
        RETURNING *`;
    ...
    const row = await prisma.$queryRaw`
        ...
        WHERE e.escalation_id = ${result[0].escalation_id}`;
    return row[0];
};
...
exports.acknowledge = async (escalationId, payload, userId) => {
    await prisma.$executeRaw`
        ...
        SET acknowledged_at = NOW(), acknowledged_by = ${userId},
            resolution_remarks = ${payload.resolution_remarks || null}
...
exports.getByClaim = async (claimId) => {
    return prisma.$queryRaw`
        SELECT e.*, r.role_name FROM insurance_claim_escalation e
        LEFT JOIN master_escalation_role r ON e.escalation_to_role_id = r.escalation_role_id
        WHERE e.claim_id = ${claimId} ORDER BY e.escalated_at DESC`;
};
...
exports.getPendingEscalations = async () => {
    return prisma.$queryRaw`
        ...
        WHERE e.acknowledged_at IS NULL AND e.escalation_due_date >= CURRENT_DATE
