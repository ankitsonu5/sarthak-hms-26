...
exports.submitClaim = async (payload, userId) => {
    return prisma.$transaction(async (tx) => {
        let slaDueDate = null;
        if (payload.tpa_id) {
            const tpa = await tx.$queryRaw`SELECT sla_days FROM master_tpa WHERE tpa_id = ${payload.tpa_id}`;
        ...
        const result = await tx.$queryRaw`
            ...
            VALUES (${payload.ipd_admission_id}, ${payload.preauth_id || null},
                    ${payload.insurance_company_id}, ${payload.tpa_id || null},
                    ${payload.claim_number || null}, ${payload.claim_amount},
                    ${payload.claim_status_id}, NOW(), ${slaDueDate})
            RETURNING *`;

        await tx.$executeRaw`
            INSERT INTO insurance_claim_tracking (claim_id, claim_status_id, remarks, updated_by)
            VALUES (${result[0].claim_id}, ${payload.claim_status_id}, 'Claim submitted', ${userId})`;
...
exports.updateClaimStatus = async (claimId, payload, userId, ipAddress = null) => {
    return prisma.$transaction(async (tx) => {
        const existing = await tx.$queryRaw`
            SELECT claim_status_id FROM insurance_claim_master WHERE claim_id = ${claimId}`;
        if (!existing.length) return null;
        const oldStatusId = existing[0].claim_status_id;

        const oldStatusRow = await tx.$queryRaw`
            SELECT status_name FROM master_claim_status WHERE claim_status_id = ${oldStatusId}`;
        const newStatusRow = await tx.$queryRaw`
            SELECT status_name FROM master_claim_status WHERE claim_status_id = ${payload.claim_status_id}`;

        const updates = ['claim_status_id = $1'];
        const params = [payload.claim_status_id];
        let idx = 2;
        if (payload.approved_amount !== undefined) {
            updates.push(`approved_amount = $${idx++}`);
            params.push(payload.approved_amount);
        }
        if (payload.settled_amount !== undefined) {
            updates.push(`settled_amount = $${idx++}`);
            params.push(payload.settled_amount);
        }
        if (payload.settlement_date) {
            updates.push(`settlement_date = $${idx++}`);
            params.push(payload.settlement_date);
        }
        params.push(claimId);

        await tx.$executeRawUnsafe(
            `UPDATE insurance_claim_master SET ${updates.join(', ')} WHERE claim_id = $${idx}`,
            ...params
        );

        await tx.$executeRaw`
            INSERT INTO insurance_claim_tracking (claim_id, claim_status_id, remarks, updated_by)
            VALUES (${claimId}, ${payload.claim_status_id}, ${payload.remarks || null}, ${userId})`;
        ...
        await tx.$executeRaw`
            ...
            VALUES (${claimId}, ${oldStatusId}, ${payload.claim_status_id},
                    ${oldStatusRow[0]?.status_name || null}, ${newStatusRow[0]?.status_name || null},
                    ${userId}, ${ipAddress || null}, ${payload.remarks || null})`;
...
exports.addRejection = async (claimId, payload) => {
    return prisma.$transaction(async (tx) => {
        await tx.$executeRaw`
            ...
            VALUES (${claimId}, ${payload.rejection_reason_id}, ${payload.rejection_amount || null}, ${payload.remarks || null})`;
...
exports.getClaimById = async (claimId) => {
    ...
    const tracking = await prisma.$queryRaw`
        ...
        WHERE ct.claim_id = ${claimId} ORDER BY ct.updated_at ASC`;
    ...
    const auditLog = await prisma.$queryRaw`
        SELECT * FROM insurance_claim_audit_log WHERE claim_id = ${claimId} ORDER BY changed_at ASC`;
...
exports.getClaimsByAdmission = async (admissionId) => {
    return prisma.$queryRaw`
        ...
        LEFT JOIN master_claim_status cs ON cm.claim_status_id = cs.claim_status_id
        WHERE cm.ipd_admission_id = ${admissionId} ORDER BY cm.created_at DESC`;
};
...
exports.getAgingSummary = async () => {
    return prisma.$queryRaw`SELECT * FROM v_insurance_claim_aging_summary`;
};
